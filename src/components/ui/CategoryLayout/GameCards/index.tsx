...
const GameCards = ({
	...
	return (
		<Card className={`w-full h-auto rounded-md space-y-2 cursor-pointer`}>
			...
			<CardBody>
				<h1 className={`text-base line-clamp-1 font-[600]`}>{name}</h1>
				...
				>
					...
					<span>
						{genres.map((genre, index) => genre.slug).join(", ")}
					</span>
				</div>
			</CardBody>
		</Card>
